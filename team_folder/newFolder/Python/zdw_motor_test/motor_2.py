...
class RoverMovement():
    ...
    def move_forward(self, reg):
        ...
        data = 1
        MemAccess(reg=reg, data_to_mem=data,
                  address_offset=front_left_encoder_address).write()
        self.front_left = 1
        MemAccess(reg=reg, data_to_mem=data,
                  address_offset=front_right_encoder_address).write()
        self.front_right = 1
        MemAccess(reg=reg, data_to_mem=data,
                  address_offset=rear_left_encoder_address).write()
        self.rear_left = 1
        MemAccess(reg=reg, data_to_mem=data,
                  address_offset=rear_right_encoder_address).write()
        self.rear_right = 1
        self.state = 1
    ...
    def rotate_left(self, reg):
        ...
        forward_data = 1
        backward_data = -1
        MemAccess(reg=reg, data_to_mem=backward_data,
                  address_offset=front_left_encoder_address).write()
        self.front_left = -1
        MemAccess(reg=reg, data_to_mem=forward_data,
                  address_offset=front_right_encoder_address).write()
        self.front_right = 1
        MemAccess(reg=reg, data_to_mem=backward_data,
                  address_offset=rear_left_encoder_address).write()
